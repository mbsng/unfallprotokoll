...
serve(async (req) => {
  ...
  try {
    ...
    if (!rawKey.startsWith("uk_live_") || rawKey.length < 32) return json(req, { error: "unauthorized" }, 401);
    ...
    if (keyError || !apiKey || apiKey.revoked_at) return json(req, { error: "unauthorized" }, 401);
    ...
    if (!allowed) return json(req, { error: "rate_limit_exceeded" }, 429);
    ...
    if (!(apiKey.scopes ?? []).includes(requiredScope)) return json(req, { error: "insufficient_scope", required_scope: requiredScope }, 403);

    if (!incidentId) {
      const rawSince = url.searchParams.get("since");
      const since = rawSince ? new Date(rawSince) : new Date(0);
      if (Number.isNaN(since.getTime())) return json(req, { error: "invalid_since" }, 400);
      ...
      if (!profileIds.length) return json(req, { schema_version: "incident.v1", items: [], next_since: new Date().toISOString() });
      ...
      if (!incidentIds.length) return json(req, { schema_version: "incident.v1", items: [], next_since: new Date().toISOString() });
      ...
      return json(req, { schema_version: "incident.v1", items, next_since: new Date().toISOString() });
    }

    if (!(await incidentBelongsToOrg(service, incidentId, apiKey.org_id))) return json(req, { error: "not_found" }, 404);
    if (pdfRoute) {
