...
serve(async (req) => {
  ...
  try {
    ...
    if (!profile?.org_id || !["insurer_agent", "admin"].includes(profile.role)) return json(req, { error: "forbidden" }, 403);
    ...
    if (action === "list") {
      ...
      return json(req, { integrations, apiKeys: keys });
    }

    if (action === "create_webhook") {
      const name = typeof body.name === "string" ? body.name.trim() : "";
      const endpointUrl = typeof body.endpointUrl === "string" ? body.endpointUrl.trim() : "";
      if (!name || name.length > 100) return json(req, { error: "invalid_webhook" }, 400);
      try {
        await validateWebhookEndpoint(endpointUrl);
      } catch {
        return json(req, { error: "invalid_webhook" }, 400);
      }
      const master = Deno.env.get("WEBHOOK_MASTER_SECRET");
      if (!master) return json(req, { error: "webhook_secret_not_configured" }, 503);
      ...
      return json(req, { integration: data, signingSecret }, 201);
    }

    if (action === "create_api_key") {
      const name = typeof body.name === "string" ? body.name.trim() : "";
      const scopes = Array.isArray(body.scopes) ? body.scopes.filter((scope: unknown) => typeof scope === "string" && allowedScopes.has(scope)) : [];
      if (!name || name.length > 100 || !scopes.length) return json(req, { error: "invalid_api_key" }, 400);
      ...
      return json(req, { apiKey: data, key: rawKey }, 201);
    }
    ...
    if (action === "revoke_api_key") {
      ...
      return json(req, { status: "revoked" });
    }
    ...
    if (action === "toggle_integration") {
      ...
      if (error) throw error;
      if (!data) return json(req, { error: "not_found" }, 404);
      return json(req, { integration: data });
    }
    ...
    if (action === "test_webhook") {
      ...
      if (!integration) return json(req, { error: "not_found" }, 404);
      ...
      if (!submission) return json(req, { error: "no_example_incident" }, 409);
      ...
      return json(req, result, response.status);
    }
    ...
    return json(req, { error: "integration_management_failed" }, 500);
  }
});
