...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof incidentId !== "string" || !/^[0-9a-f-]{36}$/i.test(incidentId)) return json(req, { error: "invalid_incident" }, 400);
    ...
    const orgAccess = profile?.org_id && ["insurer_agent", "admin"].includes(profile.role)
      ...
      : false;
    if (!ownParty && !orgAccess) return json(req, { error: "forbidden" }, 403);
    ...
    if (message === "incident_not_exportable") return json(req, { error: message }, 409);
    if (message === "incident_not_found") return json(req, { error: message }, 404);
