...
serve(async (req) => {
  ...
  try {
    ...
    if (!stripeSecret || !proPrice || !fleetPrice) return json(req, { error: "billing_not_configured" }, 503);
    ...
    if (authError || !authData.user || authData.user.is_anonymous) return json(req, { error: "unauthorized" }, 401);
    ...
    if (!profile) return json(req, { error: "profile_not_found" }, 404);
    ...
    if (plan === "fleet") {
      if (!orgId || orgType !== "fleet" || !["fleet_manager", "admin"].includes(profile.role)) return json(req, { error: "fleet_manager_required" }, 403);
    } else {
      if (orgId && orgType !== "private") return json(req, { error: "private_plan_not_available" }, 409);
      if (!orgId) {
    ...
    if (existing && ["active", "trialing"].includes(existing.status)) return json(req, { error: "already_subscribed" }, 409);
    ...
    if (!siteUrl || !/^https?:\/\//i.test(siteUrl)) return json(req, { error: "site_url_not_configured" }, 503);
