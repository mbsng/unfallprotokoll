...
import { ArrowLeft, CalendarClock, Link2, Loader2, MapPin, ShieldCheck } from "lucide-react";
...
export default function Join() {
  ...
  const { user, startAnonymous } = useAuth();
  ...
  const join = async () => {
    ...
    try {
      if (!user) {
        ...
        if (!started) throw new IncidentJoinError("unauthorized");
      }
  ...
  const occurredAt = incident?.occurredAt
    ? new Intl.DateTimeFormat(locale, { dateStyle: "long", timeStyle: "short" }).format(new Date(incident.occurredAt))
    ...
    <div className="min-h-screen bg-[#F5F7FA] text-slate-900">
      ...
      <main className="mx-auto max-w-xl px-5 py-10 md:py-16">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          ...
          {loading && <div className="flex min-h-48 items-center justify-center text-[#39719D]"><Loader2 className="h-7 w-7 animate-spin" /></div>}
          {!loading && notFound && <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm leading-relaxed text-rose-900">{t("join.notFound")}</div>}
          {!loading && incident && <>
            <p className="mt-4 leading-relaxed text-slate-600">{t("join.description")}</p>
            <div className="my-6 space-y-3 rounded-2xl bg-[#F5F8FA] p-4">
              <div><p className="text-xs font-bold uppercase tracking-wider text-slate-500">{t("join.codeLabel")}</p><p className="mt-1 font-mono text-2xl font-bold tracking-[0.18em] text-[#153B66]">{normalizedCode}</p></div>
              <div className="border-t border-slate-200 pt-3"><p className="flex items-center gap-2 text-sm font-semibold text-slate-800"><MapPin className="h-4 w-4 text-[#39719D]" />{incident.locationText || t("fields.notProvided")}</p><p className="mt-2 flex items-center gap-2 text-sm text-slate-600"><CalendarClock className="h-4 w-4 text-[#39719D]" />{occurredAt}</p></div>
            </div>
